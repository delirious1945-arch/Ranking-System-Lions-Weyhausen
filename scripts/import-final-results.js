...
async function main() {
  const snapshot = await prisma.snapshot.findFirst({
    where: { name: "Saison 2025/26 - FINAL" }
  });

  if (!snapshot) {
    console.error("Snapshot nicht gefunden!");
    return;
  }
    ...
    if (dbPlayer) {
      await prisma.snapshotPlayerValue.upsert({
        ...
        update: {
          ...
          games_won: player.won || 0,
          cnt_180: player.hs180 || 0,
          cnt_140: player.hs140 || 0,
          cnt_100: player.hs100 || 0,
          cnt_80: player.hs80 || 0,
          first_9_avg: player.f9 || 0,
          first_12_avg: player.f12 || 0,
          first_15_avg: player.f15 || 0,
          first_18_avg: player.f18 || 0
        },
        create: {
          snapshot_id: snapshot.id,
          player_id: dbPlayer.id,
      ...
      console.log(`Daten für ${dbName} aktualisiert.`);
    } else {
      console.warn(`Spieler ${player.name} nicht in DB gefunden.`);
    }
  }
}
