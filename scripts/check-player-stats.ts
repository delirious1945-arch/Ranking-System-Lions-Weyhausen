...
async function main() {
  ...
  if (matches.length > 0) {
    ...
    console.log('Singles Total:', firstHalf.filter(m => !m.isDouble).length);
    console.log('Doubles Won:', firstHalf.filter(m => m.isDouble && m.won).length);
    console.log('Doubles Total:', firstHalf.filter(m => m.isDouble).length);
    
    console.log('--- Rückrunde (10+) ---');
    console.log('Singles Won:', secondHalf.filter(m => !m.isDouble && m.won).length);
    console.log('Singles Total:', secondHalf.filter(m => !m.isDouble).length);
    console.log('Doubles Won:', secondHalf.filter(m => m.isDouble && m.won).length);
    console.log('Doubles Total:', secondHalf.filter(m => m.isDouble).length);
