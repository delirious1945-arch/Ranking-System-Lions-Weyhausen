...
export async function getAggregateStatsUpTo(upToSpieltag: number) {
    ...
    return Object.values(players).map(p => ({
        ...p,
        avg_total: p.gespielte_single_spiele > 0 ? p.avg_total_sum / p.gespielte_single_spiele : 0,
        avg_9: p.gespielte_single_spiele > 0 ? p.avg_9_sum / p.gespielte_single_spiele : 0,
        avg_18: p.gespielte_single_spiele > 0 ? p.avg_18_sum / p.gespielte_single_spiele : 0,
        siegequote_pct: p.games_played > 0 ? (p.wins / p.games_played) * 100 : 0,
        avg_high_per_leg: p.legs_total > 0 ? (p.cnt_80 + p.cnt_100 + p.cnt_140 + p.cnt_180) / p.legs_total : 0,
        gespielte_legs: p.legs_total,
        gespielte_single_spiele: p.games_played,
        sum_high_scores: (p.cnt_80 + p.cnt_100 + p.cnt_140 + p.cnt_180)
    }));
}
